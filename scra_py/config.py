...
def init_database(host, user, passwd, dbname):
    ...
    db.select_db(dbname)
    tx.execute("create table if not exists t_thread(\
        id BIGINT(12), title VARCHAR(100), author VARCHAR(30), reply_num INT(4),\
        good BOOL, status INT(4) default 0,PRIMARY KEY (id)) CHARSET=utf8mb4;")
    tx.execute("create table if not exists t_post(\
        id BIGINT(12), floor INT(4), author VARCHAR(30), content TEXT,\
        time DATETIME, comment_num INT(4), thread_id BIGINT(12),status INT(4)  default 0 ,PRIMARY KEY (id))\
        CHARSET=utf8mb4;")
    tx.execute("create table if not exists t_comment(id BIGINT(12),\
        author VARCHAR(30), content TEXT, time DATETIME, post_id BIGINT(12),status INT(4)  default 0 ,\
